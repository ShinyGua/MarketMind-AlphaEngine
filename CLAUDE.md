...
| `valuation_grader.py` | Valuation math is internally consistent (WACC>g, TV band, sensitivity center == base, intrinsic range **and `fair_value_range` both ordered low≤base≤high**, margin of safety, risk-free rate inside [0.1%, 10%]); warns if the stated `confidence` exceeds what the included method candidates support, if a live 10Y was available but the DCF fell back to the config rate, if the stored rate drifted >5bp from the live regime value, or if a **non-US/non-USD name was discounted with a US risk-free** (currency mismatch — expected `config_market:{MARKET}`) |
