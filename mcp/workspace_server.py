...
def _prompt_report_writer(ticker: str, date: str, mode: str) -> str:
    ...
    return (
        ...
        "2. **Story & Game (故事与博弈)** — the story, its size vs cap tier, certainty, stage, falsifier\n"
        "3. **Money Flow & Chip Structure (资金与筹码)** — price/returns, volume regime, chip distribution,\n"
        "   support/resistance, CN flows (主力/北向/融资/龙虎榜/股东户数) — no MA crossovers\n"
        "4. **Peer Cohort Divergence (同类股分化)** — per-peer path classes, leader, product-niche differences\n"
        ...
        "10. **Outside the Framework (框架之外)** — verbatim unconventional_factors from thesis_map\n"
